...
try {
   ...
   if ( isset($_POST['name']) ) {
      ...
      $query = "SELECT c.containerid, i.imagename, c.dockerid FROM new_v4_hyper h inner join new_v4_container c on h.hyperid = c.hyperid inner join new_v4_image i on c.imageid = i.imageid WHERE h.hypername = '" . $_POST['name'] . "'";
      //print_r($query . "\n");   
      $reponse = $bdd->query($query);
      $rows = $reponse->fetchAll();
